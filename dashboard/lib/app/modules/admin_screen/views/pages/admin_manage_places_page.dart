          ...
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                _ResponsiveStats(controller: controller, isMobile: isMobile, filter: _filter, onSelect: (f) => setState(() => _filter = f)),
                const SizedBox(height: 24),
                ...
                AdminSectionCard(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!isMobile) ...[
                        Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: _TableHeader()),
                          ...
                          child: Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text('No places match this filter.', style: GoogleFonts.inter(color: AdminColors.textSecondary)),
                              ],
                            ),
                          ),
                        )
...
class _ResponsiveStats extends StatelessWidget {
  ...
  const _ResponsiveStats({required this.controller, required this.isMobile, required this.filter, required this.onSelect});

  @override
  Widget build(BuildContext context) {
    final cards = [
      _StatBox(label: 'All', value: '${controller.totalPlaces}', color: AdminColors.textPrimary, icon: Icons.grid_view_rounded, selected: filter == null, onTap: () => onSelect(null)),
      _StatBox(label: 'Pending', value: '${controller.pendingCount}', color: AdminColors.amber, icon: Icons.schedule_rounded, selected: filter == PlaceStatus.pending, onTap: () => onSelect(PlaceStatus.pending)),
      _StatBox(label: 'Approved', value: '${controller.approvedCount}', color: AdminColors.green, icon: Icons.check_circle_outline_rounded, selected: filter == PlaceStatus.approved, onTap: () => onSelect(PlaceStatus.approved)),
      _StatBox(label: 'Rejected', value: '${controller.rejectedCount}', color: AdminColors.red, icon: Icons.cancel_outlined, selected: filter == PlaceStatus.rejected, onTap: () => onSelect(PlaceStatus.rejected)),
    ];
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: widget.selected
                  ...
                  : (_hovering ? widget.color.withOpacity(0.4) : AdminColors.border),
                  ...
                  children: [
                    ...
                    TweenAnimationBuilder<int>(
                      ...
                      builder: (context, value, _) => Text(
                        '$value',
                        style: GoogleFonts.spaceGrotesk(fontSize: 22, fontWeight: FontWeight.w700, color: widget.color),
                      ),
                    ),
                  ],
                ),
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: selected
                ? [BoxShadow(color: AdminColors.primary.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 4))]
                : [],
          ),
          child: Text(
            widget.label,
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(place.name,
                              style: GoogleFonts.spaceGrotesk(fontSize: 13.5, fontWeight: FontWeight.w600, color: AdminColors.textPrimary),
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis),
                          Text(place.subtitle,
                              style: GoogleFonts.spaceGrotesk(fontSize: 12, color: AdminColors.textSecondary),
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis),
                        ],
                      ),
            ...
            Expanded(flex: 2, child: Padding(padding: const EdgeInsets.only(right: 12), child: Text(place.company, style: GoogleFonts.inter(fontSize: 13.5, color: AdminColors.textPrimary), overflow: TextOverflow.ellipsis))),
            Expanded(
              flex: 2,
              child: Padding(
                padding: const EdgeInsets.only(right: 12),
                child: Align(
                  ...
                  child: Container(
                    ...
                    child: Text(place.category, style: GoogleFonts.inter(fontSize: 12, color: AdminColors.textPrimary), overflow: TextOverflow.ellipsis),
                  ),
                ),
              ),
            ),
            Expanded(flex: 1, child: Padding(padding: const EdgeInsets.only(right: 12), child: Text(place.province, style: GoogleFonts.inter(fontSize: 13.5, color: AdminColors.textPrimary), overflow: TextOverflow.ellipsis))),
            Expanded(
              flex: 1,
              child: Padding(
                padding: const EdgeInsets.only(right: 12),
                child: Text(place.fee, style: GoogleFonts.inter(fontSize: 13.5, fontWeight: FontWeight.w600, color: AdminColors.primary), overflow: TextOverflow.ellipsis),
              ),
            ),
            Expanded(flex: 1, child: Padding(padding: const EdgeInsets.only(right: 12), child: Text(place.submittedDate, style: GoogleFonts.inter(fontSize: 13.5, color: AdminColors.textPrimary), overflow: TextOverflow.ellipsis))),
            Expanded(
              flex: 2,
              child: Padding(
                padding: const EdgeInsets.only(right: 12),
                child: Align(
                  alignment: Alignment.centerLeft,
                  child: FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: _statusChip(place.status)),
                ),
              ),
            ),
            Expanded(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  _ActionIconButton(icon: Icons.check_circle_outline, tooltip: 'Approve', color: AdminColors.green, onTap: () => controller.approvePlace(place)),
                  _ActionIconButton(icon: Icons.cancel_outlined, tooltip: 'Reject', color: AdminColors.red, onTap: () => controller.rejectPlace(place)),
                  _ActionIconButton(icon: Icons.delete_outline, tooltip: 'Delete', color: AdminColors.textSecondary, onTap: () => _confirmDelete(context, place, controller)),
                ],
              ),
            ),
          ],
